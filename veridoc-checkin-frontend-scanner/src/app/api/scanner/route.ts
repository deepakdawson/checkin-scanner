...
import { QrCodeDetailsConfirmRequest } from "@/src/models/visitor/scannerModels";
...
export async function POST(request: NextRequest) {
    ...
    const data = (await request.json()) as QrCodeDetailsConfirmRequest;
    data.visitorId = payload?.nameid ?? '';
    try {
        const res = await httpClient.post('/visitor/scan-details', data, session.user.accessToken);
